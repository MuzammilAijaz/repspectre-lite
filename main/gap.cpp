...
static void start_advertising(void) {
    ...
    int rc = 0;
    const char *name;
    struct ble_hs_adv_fields adv_fields{}; // holds advertisements
    struct ble_hs_adv_fields rsp_fields{}; // holds responses
    ...
    /* Set advertising interval */ // NOTE : This DOES NOT change the actual interval, just informs client
    rsp_fields.adv_itvl = BLE_GAP_ADV_ITVL_MS(500); // BLE_GAP_ADV_ITVL_MS converts 500ms into adv units
    ...
    /* Set advertising interval */ // NOTE : This actually changes the interval
    adv_params.itvl_min = BLE_GAP_ADV_ITVL_MS(500); 
    adv_params.itvl_max = BLE_GAP_ADV_ITVL_MS(510);

    /* Start advertising */
    rc = ble_gap_adv_start(own_addr_type, NULL, BLE_HS_FOREVER, &adv_params,
                           gap_event_handler, NULL); // calls handler everytime a gap event is triggered
    if (rc != 0) {
...
static int gap_event_handler(struct ble_gap_event *event, void *arg) {
    ...
    // ---- If the user Enables INDICATIONS/NOTIFICATIONS ----
    /* Subscribe event */
    case BLE_GAP_EVENT_SUBSCRIBE:
        ...
        if (event->type == BLE_GAP_EVENT_SUBSCRIBE) {
            if (event->subscribe.cur_notify && !event->subscribe.cur_indicate) {
                ...
                ESP_LOGW(TAG, "XXXXXXXXXXXXXXXXXXXXXX Client subscribed to INDICATIONS XXXXXXXXXXXXXXXXXXXXX ");
            }
        }

        /* GATT subscribe event callback
         * handles the subscription event, which is done by the GATT 
         * side of the code in order to keep the logic seperated
         */
